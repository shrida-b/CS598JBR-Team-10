...
def prompt_model(dataset, model_name = "deepseek-ai/deepseek-coder-6.7b-base", quantization = True):
    ...
    for case in dataset:
        ...
        if "```python" in response_processed:
            ...
            response_processed = parts[1] if response_processed.lstrip().startswith("```") else parts[0]

        lines = response_processed.split("\n")
        cleaned_lines = []
        for line in lines:
            stripped = line.strip()
            # Stop when main execution blocks or test runners start
            if stripped.startswith(("if __name__", "import unittest", "def main():", "<jupyter")):
                break
            cleaned_lines.append(line)

        candidate_code = "\n".join(cleaned_lines).rstrip()

        # Check AST parse with the prompt attached
        temp_code = candidate_code
        while temp_code:
            try:
