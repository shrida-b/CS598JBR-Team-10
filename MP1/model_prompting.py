...
def prompt_model(dataset, model_name = "deepseek-ai/deepseek-coder-6.7b-base", quantization = True):
    ...
    for case in dataset:
        ...
        for line in lines:
            stripped = line.strip()
            # Stop if another top-level function/class starts
            if cleaned_lines and (line.startswith("def ") or line.startswith("class ")):
                break
            # Strip top-level execution calls, test prints, or jupyter outputs appended at the end
            if line and not line.startswith(" ") and not line.startswith("\t"):
                if stripped.startswith(("print(", "print ", "if __name__", "import unittest", "<jupyter")):
                    break
            cleaned_lines.append(line)

        # Truncate trailing syntax errors (e.g. unclosed brackets in truncated prints)
        while cleaned_lines:
            try:
                ast.parse("\n".join(cleaned_lines))
                response_processed = "\n".join(cleaned_lines)
